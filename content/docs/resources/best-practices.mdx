...
2. **Transformación a JSON**: Implementa una capa que convierta tus datos al [JSON de e-CF](/docs/integration/document-format). MSeller lo convierte a XML y lo firma.
...
- Implementa pruebas unitarias para verificar la correcta generación de los documentos JSON.
- Usa `?validate=true` en TesteCF para detectar errores antes de enviar a la DGII.
