---
title: "Envío de e-CF"
seoTitle: "Enviar e-CF a la DGII por API: Ejemplos JSON"
description: "Cómo emitir y enviar comprobantes fiscales electrónicos (e-CF) a la DGII con un POST JSON, y cómo leer la respuesta, con ejemplos."
---

<Callout title="En resumen">
  1. Haz `POST /{entorno}/documentos-ecf` con el JSON del comprobante ([formato](/docs/integration/document-format), [ejemplos por tipo](/docs/integration/formato-documentos-ecf)).
  2. Recibes al instante el `securityCode` y el `qr_url` para la factura impresa.
  3. Unos segundos después, [consulta el estado](/docs/integration/document-queries) para saber si la DGII lo aceptó.

  Durante el desarrollo usa `?validate=true` para detectar errores sin enviar a la DGII ([ver más](#parámetro-de-validación)).
</Callout>
...
- **Soporte técnico de MSeller**: A través del [portal de soporte](https://ecf.mseller.app/support)
- **Comunidad**: El [Discord eCF-Dominicana](https://discord.gg/7xnNByfWTG)
